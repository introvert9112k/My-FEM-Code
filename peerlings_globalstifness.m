% global stiffness
function [Stif,fai,fe,D_st,kappa,NE_gp,stress_gp,interaction,stress_gp_sm,eq_stress,neq_stress,strain_gp_local,strain_gp_non_local,eq_strain] = peerlings_globalstifness(u_tot,strain_tot,D_st,material_p, ...
...
for iel = 1:numelem % Loop on elements  
    ...
    for kk = 1 : size(W,1) 
        ...
        D_i = D_st(gpnt,1);
        Omega = Compute_damage_using_peerlings_law(C,alpha,beta,eps_nl,D_i,kappa_gpt,kappa0_gpt);
...
%         sm_stress_gpt = De*strain_gpt_nonlocal; % Smooth Stress Vector
        ...
        elseif e_tilde1 < eps_nl 
            DomegaDeps_nl = C*exp(alpha*Omega)*beta*(eps_nl)^(beta-1);
        else 
            DomegaDeps_nl = 0;
        end 
...
%         end
        ...
        Y2 = (dSigma_dStrain + dSigma_dOmega* DomegaDeps_nl *deps_de_nl)*N2;
        X1 = dCbar_dSigma;
        ...
        Coff3 = -B1'*(hcoup + De*strain_gpt_local* DomegaDeps_nl *deps_de_nl)*N2; %Coffiecent of Kue
        ...
        Coff6 = B2'*hcoup*C_grad*B2 + N2'*hcoup*N2 + B2'*hcoup*c_len*g_int*dcdu + B2'*hcoup*Cbar*del_strain_gpt_nl*dgdomega* DomegaDeps_nl*deps_de_nl*N2; %coffiecent of kee
